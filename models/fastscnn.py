...
class Fast_SCNN_Border(torch.nn.Module):
    def __init__(self, input_channel, num_classes):
        super().__init__()
        ...
        self.feature_fusion = FeatureFusionModule()
        self.classifier = Classifier(num_classes)
        self.border_classifier = Classifier(num_classes)
    ...
    def forward(self, x):
        ...
        border = self.border_classifier(x)
        seg = F.interpolate(seg, size=size, mode='bilinear', align_corners=True)
        border = F.interpolate(border, size=size, mode='bilinear', align_corners=True)
        return seg, border
...
if __name__ == '__main__':
    # m = Fast_SCNN(input_channel=3, num_classes=19).cuda(1)
    m = LearningToDownsample(in_channels=3).cuda(1)
    # a = torch.randn(1, 3, 1024, 2048).cuda(1)
    m = GlobalFeatureExtractor().cuda(1)
    b = torch.randn([1, 64, 128, 256]).cuda(1)
    # a = torch.randn(1, 3, 512, 1024).cuda(1)
