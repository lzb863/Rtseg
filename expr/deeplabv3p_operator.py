...
from models.dplabv3p import DeepLab
...
class DeeplabV3PlusOperator(BaseOperator):
    def __init__(self, cfg):
        super(DeeplabV3PlusOperator, self).__init__(cfg)
        # prepare model for train
        self.model = DeepLab(backbone='resnet', output_stride=16, sync_bn=True, num_classes=self.cfg.Train.num_classes).cuda(self.cfg.Distributed.gpu_id)
        if self.cfg.Distributed.dist:
            self.model = DistributedDataParallel(self.model, device_ids=[self.cfg.Distributed.gpu_id])

        self.optimizer = torch.optim.Adam(params=[
            {'params': get_params(self.model, key='1x'), 'lr': cfg.Train.learning_rate},
            {'params': get_params(self.model, key='10x'), 'lr': 10 * cfg.Train.learning_rate}
        ], lr=self.cfg.Train.learning_rate, weight_decay=self.cfg.Train.weight_decay)
    ...
    def eval_process(self):
        self.model.eval()
        # self.model.module.load_state_dict(self.cfg.val.model_path)
        epoch = 0
        step = 0
...
if __name__ == '__main__':
    ...
    trainer = DistributedWrapper(config, DeeplabV3PlusOperator)
    trainer.train()
    # trainer.eval()
